        ...
        {images.length === 0 ? (
          <div className="text-center py-16 bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <p className="text-slate-500">
              گالری تصاویر مهدکودک در حال آماده‌سازی است.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="absolute bottom-0 right-0 left-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform">
                  ...
                  <span className="text-white/80 text-sm">
                    {item.category === "classes" && "کلاس‌ها"}
                    {item.category === "facilities" && "امکانات"}
                    {item.category === "activities" && "فعالیت‌ها"}
                    {item.category === "events" && "رویدادها"}
                    {item.category === "general" && "عمومی"}
                  </span>
                </div>
              </div>
            ))}
